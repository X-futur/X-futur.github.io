...
<b>项目实现</b>：本项目设计了高效的文本分类模型。在数据处理阶段，将文本数据通过`填充或截取`统一长度为256的样本。同时在 `FNet`的基础上添加了残差块以解决深层网络训练时出现的梯度消失问题，实现了较为完善的`嵌入层-层归一化层-傅里叶变换层-前馈网络层-输出层`架构。最终模型在测试集上文本分类准确率达到`87.18%`，损失降低到了`34.61%`。<br>
<b>个人工作</b>：负责整个模型框架的搭建与模型的训练。<br>
<b>项目成果展示</b>：<br>
<img src='../img/FNet-accuracy.jpg' width='400' title='FNet模型训练accuracy-epoch曲线'>
<img src='../img/FNet-loss.jpg' width='400' title='FNet模型训练loss-epoch曲线'>
<br><br>
